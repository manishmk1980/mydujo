...
export default function InstructorLoginPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const { error: loginError } = await login(emailTrimmed, password, 'instructor');

      if (loginError) {
        setError(
          typeof loginError === 'string'
            ? loginError
            : loginError?.message || 'Instructor login failed.'
        );
        return;
      }

      navigate(from || '/instructor', { replace: true });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const sidePanel = (
    <div className="rounded-3xl border border-slate-200/80 bg-white/70 p-6 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.05] sm:p-7">
      ...
      <p className="mt-3 text-sm font-medium leading-6 text-slate-600 dark:text-white/70 sm:text-base sm:leading-7">
        Apply as an instructor to manage students, attendance, grading, and academy operations on MDPL MyDojo.
      </p>
  ...
  return (
    <PublicAuthLayout
      ...
      title={
        <>
          Welcome
          <span className="block text-orange-600">back, instructor</span>
        </>
      }
      description="Access academy operations, student records, attendance, grading tools, and more."
      sidePanel={sidePanel}
    >
      <PublicAuthCard
        eyebrow="Instructor Sign In"
        title="Sign in to your portal"
        description="Use the email and password issued by MDPL when your instructor account was approved."
      >
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          ...
          <div className="space-y-2">
            <label htmlFor="instructor-email" className="ml-1 text-[11px] font-black uppercase tracking-[0.2em] text-slate-700 dark:text-white/80">
              Email
            </label>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-2">
              <label htmlFor="instructor-password" className="ml-1 text-[11px] font-black uppercase tracking-[0.2em] text-slate-700 dark:text-white/80">
                Password
              </label>
              <Link
                to="/forgot-password"
          ...
          >
            ...
            {submitting ? 'Signing in...' : 'Sign in as instructor'}
          ...
          <div className="flex flex-col items-stretch gap-3 border-t border-slate-100 pt-5 text-center text-sm dark:border-white/10 sm:flex-row sm:items-center sm:justify-between sm:text-left">
            <p className="text-sm font-medium text-slate-600 dark:text-white/70">
              ...
              <Link to="/register/instructor" className="font-black text-orange-600 hover:underline dark:text-orange-400">
                Apply now
              </Link>
            </p>
            <Link
